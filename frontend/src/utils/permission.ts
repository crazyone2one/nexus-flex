...
import type {SystemScopeType, UserRole, UserRoleRelation} from "/@/api/types/auth.ts";
...
export const topLevelMenuHasPermission = (route: RouteLocationNormalized | RouteRecordRaw) => {
    ...
    if (userState.lastProjectId === 'no_such_project' || userState.lastProjectId === '') {
        // 项目不存在, 不显示任何项目级别菜单, 展示无资源页面
        return false;
    }
    if (appState.currentMenuConfig.length && !appState.currentMenuConfig.includes(route.name as string)) {
        // 没有配置的菜单不显示
        return false;
    }
    if (userStore.isAdmin) {
