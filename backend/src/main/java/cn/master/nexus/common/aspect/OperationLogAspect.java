...
public class OperationLogAspect {
    ...
    @Pointcut("@annotation(cn.master.nexus.common.annotation.Log)")
    ...
    public void before(JoinPoint joinPoint) {
        try {
            localUser.set(SessionUtils.getUserName());
            ...
            if (msLog != null && isMatch(msLog.type())) {
                // 获取参数对象数组
                EvaluationContext context = getEvaluationContext(joinPoint, method);
        ...
        try {
            ...
            switch (obj) {
                case List<?> _ -> {
                    ...
                    List<LogDTO> logDTOs = (List<LogDTO>) obj;
                    mergeLists(beforeValues.get(), logDTOs);
                }
                case LogDTO log -> {
